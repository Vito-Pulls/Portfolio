...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $bd->prepare(
            "UPDATE posts SET titulo=?, slug=?, resumen=?, contenido=?, publicado=? WHERE id=?"
        );
        $stmt->bind_param('ssssii', $titulo, $slug, $resumen, $contenido, $publicado, $id);
        $stmt->execute();
        $stmt->close();
        $bd->close();
        ...
        <form method="POST" action="<?= BASE_URL ?>/admin/editPost.php?id=<?= $id ?>" class="contact-form admin-form">
            ...
            <div class="form-grupo">
                ...
                <textarea class="form-input form-textarea admin-textarea" id="contenido" name="contenido"
                    rows="14"><?= htmlspecialchars($post['contenido']) ?></textarea>
